    ...
    impl Display for PartiallySignedTransaction {
        fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
            write!(
                f,
                "{}",
                Base64Display::with_config(&self.serialize(), base64::STANDARD)
            )
        }
    }
...
mod tests {
    ...
    mod bip_371_vectors {
        ...
        fn invalid_vectors() {
            ...
            assert_eq!(
                err.to_string(),
                "hash parse error: invalid slice length 33 (expected 32)"
            );
    ...
    fn combine_psbts_commutative() {
        ...
        psbt1
            .combine(psbt2_clone)
            .expect("psbt1 combine to succeed");
        psbt2
            .combine(psbt1_clone)
            .expect("psbt2 combine to succeed");
