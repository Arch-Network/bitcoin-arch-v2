...
#[cfg(feature = "borsh")]
impl borsh::ser::BorshSerialize for OutPoint {
    #[inline]
    fn serialize<W: borsh::io::Write>(&self, writer: &mut W) -> borsh::io::Result<()> {
        let mut s = self.txid.to_string();
        s.push(':');
        s.push_str(self.vout.to_string().as_str());
        s.serialize(writer)
    }
}
#[cfg(feature = "borsh")]
impl borsh::de::BorshDeserialize for OutPoint {
    fn deserialize_reader<R>(buf: &mut R) -> Result<Self, borsh::io::Error>
    where
        R: borsh::io::Read,
    {
        let outpoint = borsh::from_reader::<_, String>(buf).map_err(|_| {
            borsh::io::Error::new(
                borsh::io::ErrorKind::Other,
                "failed to deserialize OutPoint",
            )
        })?;
        let mut split = outpoint.split(":");
        let txid = split.next().ok_or(borsh::io::Error::new(
            borsh::io::ErrorKind::Other,
            "Invalid Outpoint serialization",
        ))?;
        let vout = split.next().ok_or(borsh::io::Error::new(
            borsh::io::ErrorKind::Other,
            "Invalid Outpoint serialization",
        ))?;
        let txid = Txid::from_str(txid)
            .map_err(|_| borsh::io::Error::new(borsh::io::ErrorKind::Other, "Invalid txid"))?;
        let vout = vout
            .parse::<u32>()
            .map_err(|_| borsh::io::Error::new(borsh::io::ErrorKind::Other, "Invalid vout"))?;
        Ok(Self { txid, vout })
    }
}
...
impl Transaction {
    ...
    #[deprecated(
        since = "0.30.0",
        note = "Use SighashCache::legacy_signature_hash instead"
    )]
...
mod tests {
    ...
    fn test_outpoint() {
        ...
        assert_eq!(
            ...
            Err(ParseOutPointError::Vout(
                crate::parse::int::<u32, _>("lol").unwrap_err()
            ))
        );
    ...
    fn test_nonsegwit_transaction() {
        ...
        assert_eq!(
            realtx.check_weight().to_wu() as usize,
    ...
    fn test_huge_witness() {
        deserialize::<Transaction>(&hex!(
            include_str!("../../tests/data/huge_witness.hex").trim()
        ))
        .unwrap();
    }
