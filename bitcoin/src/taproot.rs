...
use crate::{CryptoError, InvalidParityValue, Parity};
...
impl TaprootSpendInfo {
    ...
    /// [`TaprootBuilder`].
    pub fn from_node_info(internal_key: UntweakedPublicKey, node: NodeInfo) -> TaprootSpendInfo {
        // Create as if it is a key spend path with the given merkle root
        let root_hash = Some(node.hash);
        let mut info = TaprootSpendInfo::new_key_spend(internal_key, root_hash);
...
impl ControlBlock {
    ...
    pub fn verify_taproot_commitment(&self, output_key: XOnlyPublicKey, script: &Script) -> bool {
...
pub enum TaprootBuilderError {
    ...
    InvalidInternalKey(CryptoError),
...
pub enum TaprootError {
    ...
    InvalidParity(InvalidParityValue),
    /// Empty tap tree.
    EmptyTree,
}
...
mod test {
    ...
    fn test_midstates() {
        ...
        assert_eq!(
            MIDSTATE_TAPLEAF,
            tag_engine("TapLeaf").midstate().to_byte_array()
        );
        assert_eq!(
            MIDSTATE_TAPBRANCH,
            tag_engine("TapBranch").midstate().to_byte_array()
        );
        assert_eq!(
            MIDSTATE_TAPTWEAK,
            tag_engine("TapTweak").midstate().to_byte_array()
        );
        assert_eq!(
            MIDSTATE_TAPSIGHASH,
            tag_engine("TapSighash").midstate().to_byte_array()
        );
    ...
    fn build_huffman_tree() {
        ...
        for (_weights, script) in script_weights {
            ...
            assert!(ctrl_block.verify_taproot_commitment(output_key.to_inner(), &ver_script.0))
        }
    }
    ...
    fn bip_341_tests() {
        ...
        for arr in data["scriptPubKey"].as_array().unwrap() {
            ...
            let addr = Address::p2tr(internal_key, merkle_root, Network::Bitcoin);
