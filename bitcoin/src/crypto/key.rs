...
impl PublicKey {
    ...
    /// Assumes the private key is compressed
...
impl CompressedPublicKey {
    ...
    /// Computes the public key as supposed to be used with this secret
    pub fn from_private_key(sk: &k256::SecretKey) -> Self {
        Self(sk.public_key())
    }
...
impl PrivateKey {
    ...
    pub fn new(key: k256::SecretKey, network: impl Into<NetworkKind>) -> PrivateKey {
        PrivateKey {
            compressed: true,
    ...
    pub fn new_uncompressed(key: k256::SecretKey, network: impl Into<NetworkKind>) -> PrivateKey {
        PrivateKey {
            compressed: false,
            network: network.into(),
            inner: key,
    ...
    ) -> Result<PrivateKey, k256::elliptic_curve::Error> {
        Ok(PrivateKey::new(k256::SecretKey::from_slice(data)?, network))
    }
...
mod tests {
    use super::*;
    use crate::{address::Address, CryptoError};
    ...
    fn public_key_from_str_invalid_str() {
        ...
        assert_eq!(
            res.unwrap_err(),
            ParsePublicKeyError::Encoding(FromSliceError::Secp256k1(CryptoError::InvalidPublicKey))
        );
